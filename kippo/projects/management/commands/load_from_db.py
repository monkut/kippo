...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser: ArgumentParser):
        parser.add_argument("-d", "--dbname", type=str, default=None, required=True, help=_("Database name to migrate data from"))
        parser.add_argument(
            "--host",
            default="127.0.0.1",
        )
    ...
    def handle(self, *args, **options):  # noqa: C901, PLR0915, PLR0912
        ...
        with psycopg2.connect(**params) as conn:  # noqa: SIM117
            with conn.cursor(cursor_factory=psycopg2.extras.DictCursor) as cursor:  # noqa: SIM117
                # load users
                table_name = "accounts_kippouser"
                cursor.execute("SELECT * from %s WHERE username NOT IN ('admin', 'github-manager', 'cli-manager')", (table_name,))
