...
class ClockInSubCommandTestCase(IsStaffModelAdminTestCaseBase):
    ...
    def test_attendanceslackmanager_processcommand_breakstartcommand_aliases(self, *_):
        ...
        for alias in BreakStartSubCommand.ALIASES:
            payload = {
            ...
            self.assertEqual(AttendanceRecord.objects.filter(category=AttendanceRecordCategory.BREAK_START).count(), expected_attendancerecord_count)
    ...
    def test_with_preexisting_attendancerecord(self, *_):
        ...
        command = SlackCommand(
            ...
            sub_command="break-start",
            text="break-start 休憩しますよ",
            response_url="https://example.com/response_url",
        )
        command.save()
    ...
    def test_set_by_datetime(self, *_):
        ...
        subcommand_text = f"break-start {expected_entry_datetime.strftime('%Y-%m-%d %H:%M')}"
        command = SlackCommand(
            organization=self.organization,
            user=self.staffuser_with_org,
            sub_command="break-start",
        ...
        actual_end_attendancerecord_count = AttendanceRecord.objects.filter(category=AttendanceRecordCategory.BREAK_START).count()
        self.assertEqual(actual_end_attendancerecord_count, expected_end_attendancerecord_count)

        # check datetime is as expected
        end_attendance_record = AttendanceRecord.objects.filter(category=AttendanceRecordCategory.BREAK_START).first()
    ...
    def test_set_by_datetime_without_year(self, *_):
        ...
        for separator in ("/", "-"):
            ...
            subcommand_text = f"break-start {expected_entry_datetime.strftime(date_format)}"
            command = SlackCommand(
                organization=self.organization,
                user=self.staffuser_with_org,
                sub_command="break-start",
