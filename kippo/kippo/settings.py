...
INSTALLED_APPS = [
    ...
    "drf_spectacular",
    "nested_admin",  # nested inlines (BillingEntry under Contract on the KippoProject admin)
]
...
# Stage prefix: on the deployed API Gateway stage the browser requests `/prod/static/ui/...`
# but API Gateway strips the stage before the request reaches Django, and WhiteNoise keys
# every file by WHITENOISE_STATIC_PREFIX (`/static/`, pinned above) + relative path — so the
# URL passed to this test is `/static/ui/assets/...` with no stage. The leading `^.*` accepts
# both the stripped form and a prefixed one; the Django-hash alternative is prefix-agnostic.
...
LOGGING = {
    ...
    "loggers": {
        ...
        "commons": {"handlers": ["console"], "level": DJANGO_LOG_LEVEL, "propagate": False},
        "projects": {"handlers": ["console"], "level": DJANGO_LOG_LEVEL, "propagate": False},
...
DEFAULT_KIPPOPROJECT_CATEGORY = "other"
...
# used to compare the exceeding % of the project status report (0-100) if exceeding the threshold, :red_circle: will be used
DEFAULT_PROJECT_STATUS_REPORT_EXCEEDING_THRESHOLD = "15"
PROJECT_STATUS_REPORT_EXCEEDING_THRESHOLD = int(
    os.getenv("PROJECT_STATUS_REPORT_EXCEEDING_THRESHOLD", DEFAULT_PROJECT_STATUS_REPORT_EXCEEDING_THRESHOLD)
)

# Number of days to keep the slack image URLs in the database before refreshing them
DEFAULT_REFRESH_SLACK_IMAGE_URL_DAYS = "15"
REFRESH_SLACK_IMAGE_URL_DAYS = int(os.getenv("REFRESH_SLACK_IMAGE_URL_DAYS", DEFAULT_REFRESH_SLACK_IMAGE_URL_DAYS))

DEFAULT_ATTENDANCECANCEL_SUBCOMMAND_MINUTES = "5"
ATTENDANCECANCEL_SUBCOMMAND_MINUTES = int(os.getenv("ATTENDANCECANCEL_SUBCOMMAND_MINUTES", DEFAULT_ATTENDANCECANCEL_SUBCOMMAND_MINUTES))
