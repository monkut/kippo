...
def _first_of_next_month(reference: datetime.date) -> datetime.date:
...
def _build_response(project: KippoProject, completion_date: datetime.date | None) -> dict:
    delta = None
    if project.target_date and completion_date:
        delta = (completion_date - project.target_date).days
    return {
...
def _logged_hours_through(project: KippoProject, today: datetime.date) -> int:
    return ProjectWeeklyEffort.objects.filter(project=project, week_start__lte=today).aggregate(total=Sum("hours"))["total"] or 0


def _latest_logged_week_start(project: KippoProject, today: datetime.date) -> datetime.date | None:
    latest = ProjectWeeklyEffort.objects.filter(project=project, week_start__lte=today).order_by("-week_start").first()
...
def _user_contributes_on(ctx: _UserContext, user_id: int, day: datetime.date, weekday: int) -> int | None:
    ...
    *,
    ...
    while current_day <= horizon:
        ...
        for user_id in ctx.by_user_month:
            percentage = _user_contributes_on(ctx, user_id, current_day, weekday)
            if percentage is None:
                continue
...
def compute_estimated_completion(project: KippoProject) -> dict:
    """Estimate the completion date for `project`.
    ...
    if not allocated_hours:
        return _build_response(project, None)

    today = timezone.localdate()
    spent_hours = _logged_hours_through(project, today)

    if spent_hours >= allocated_hours:
        return _build_response(project, _latest_logged_week_start(project, today) or today)
    ...
    ctx = _load_user_context(project, future_assignments, next_month_start, horizon)
    completion = _walk_to_completion(
        ctx=ctx,
    ...
    return _build_response(project, completion)
